...
const OrderManagement = () => {
  ...
  const fetchOrders = async (customerId) => {
    try {
      const response = await fetch(
        `https://localhost:7055/api/Order/OrderSearch?CustomeriD=${customerId}`
      );
      if (!response.ok) {
  ...
  const handleRebooking = async (orderId) => {
    try {
      const response = await axios.put(
        `https://localhost:7055/api/Order/ReOrderProduct?orderId=${orderId}`
      );
  ...
  const updateOrderStatus = async (orderId, newStatus) => {
    ...
    try {
      await axios.put(
        "https://localhost:7055/api/Order/UserAcceptedProductDelivered",
        null,
        {
          params: {
            OrderId: orderId,
  ...
  const handleSubmitComment = async (e, productId) => {
    ...
    try {
      const response = await fetch(
        "https://localhost:7055/api/FeedBack/Create",
        {
          method: "POST",
          headers: {
  ...
  const userOrders = orders.filter(
    (order) =>
      order.customerId === currentUser.userId && order.orderId.includes("ORP")
  );
      ...
      stars.push(
        <StarIcon
          ...
          className={`h-5 w-5 cursor-pointer ${
            i <= commentStar ? "text-yellow-400" : "text-gray-300"
          }`}
  ...
  return (
    <div>
      ...
      {error ? (
                ...
                >
                                ...
                                >
                                  <td className="pr-6 pl-24 whitespace-nowrap text-sm text-gray-500">
                                    <div className="flex items-center">
                                      <img
                                        className="h-24 w-32 mr-4"
                                        src={detail.pictureName}
                  ...
                  {orderDetails.map((detail) => (
                    ...
                    >
                      ...
                      <form
                        onSubmit={(e) =>
                          handleSubmitComment(e, detail.productId)
                        }
                        className="mt-4 space-y-4"
                      >
                        <div>
                          <label
                            htmlFor={`commentText-${detail.productId}`}
        ...
        <p className="mt-4">
          Không có đơn hàng nào liên quan đến người dùng hiện tại.
        </p>
      )}
    </div>
  );
